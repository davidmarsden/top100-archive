...
const isLegacyPredictionHeader = (row) => {
  ...
  // S2D5 style has no written Club/Pre headers: blank, blank, VA.
  const hasBlankClubAndPrediction = !keys[0] && !keys[1] && hasValueAdded;

  return (hasPrediction && (hasActual || hasValueAdded)) || hasBlankClubAndPrediction;
};
